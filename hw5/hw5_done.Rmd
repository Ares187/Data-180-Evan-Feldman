...
```{r}
Vans_Dist <- dist((Vans93[, -1]), method = 'euclidian')
Vans_Euclidian <- as.matrix(Vans_Dist)
colnames(Vans_Euclidian)<-Vans93[,1]
rownames(Vans_Euclidian)<-Vans93[,1]
Vans_Euclidian <- round(Vans_Euclidian, 2)
print(Vans_Euclidian)

#The closest distance is caravan and aerostar
#the furthest distance is silhouette and previa
...
```{r}
Vans_Dist_M <- dist((Vans93[, -1]), method = 'manhattan')
Vans_Manhatten <- as.matrix(Vans_Dist)
colnames(Vans_Manhatten)<-Vans93[,1]
rownames(Vans_Manhatten)<-Vans93[,1]
Vans_Manhatten <- round(Vans_Manhatten, 2)
```
...
The nearest neighbor cannot be further than the furthest neighbor.
a) Nearest-neighbor
...
#C1 = {in, on}, C2 = {with, for}, C3 = {at, from}, C4 = {and,but}, C5 = {of,to,as}
#These groupings are chosen in order of earliest to latest of when they get paired.
...
plot(hclust(dist(Ver_Euclidean[,-1], method='euclidian'), method = 'complete'))
...
colnames(Ver_Euclidean)<-vertebrate[,1]
rownames(Ver_Euclidean)<-vertebrate[,1]
plot(hclust(dist(Ver_Euclidean[,-1], method='euclidian'), method = 'single'))
```

Which method do you prefer?

#Complete linkage gives a more clear and easy to read graph with more variation in heights
...
colnames(Ver_Euclidean)<-vertebrate[,1]
rownames(Ver_Euclidean)<-vertebrate[,1]
plot(hclust(dist(Ver_Euclidean[,-1], method='euclidian'), method = 'complete'), lwd = 3, main = 'Clustered vertebrates, height, complete linkage', sub = '', xlab = 'Vertebrates', cex.lab = 1.25, hang = -1)
