...
```{r}
records %>% group_by(type) %>%
  summarize(avg_Period = mean(time_period), med_Period = median(time_period), avg_Time = mean(time), med_Time = median(time))
```
# Question 2
...
```{r}
records %>%
  dplyr::select(player, track, date, record_duration) %>%
...
records %>%
  ...
  labs(title = "Distribution of records by track type", x = 'time in seconds', y = 'frequency')
```
...
```{r}
records <- records %>%
  mutate(time_period = ifelse(type == 'three laps', time/3, time))

records %>%
  ggplot(aes(x = time_period, fill = type)) +
  geom_histogram(position = 'identity', alpha = .5, bins = 30) +
  labs(title = 'time per lap by track type', x = 'time per lap', y = 'frequency')
```
# Question 10
...
```{r}
records <- records %>%
  mutate(time_period = ifelse(type == 'three lap', time / 3, time))
...
#NTSC system has a significantly faster average time, meaning higher fps = faster races
